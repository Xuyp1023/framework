...
public class Runnable implements java.lang.Runnable {
    ...
    private static String mappingPath = "sqlmap"; // xml文件夹匹配字符串，需要根据需要修改
    ...
    public Runnable(final String location, final Configuration configuration) {
        this.location = location != null ? location.replaceAll("\\\\", "/") : null;
     ...
     *            文件未找到
     */
    public void refresh(final String filePath, final Long beforeTime) throws Exception {
        if (filePath == null) {
            return;
        }

        // 本次刷新时间
        final Long refrehTime = System.currentTimeMillis();

        final List<File> refreshs = this.getRefreshFile(new File(filePath), beforeTime);
        if (refreshs.size() > 0) {
            log.debug("refresh files:" + refreshs.size());
        }
        for (int i = 0; i < refreshs.size(); i++) {
            System.out.println("Refresh file: " + mappingPath
                    + StringUtils.substringAfterLast(refreshs.get(i).getAbsolutePath(), mappingPath));
            log.debug("refresh file:" + refreshs.get(i).getAbsolutePath());
            log.debug("refresh filename:" + refreshs.get(i).getName());
            SqlSessionFactoryBean.refresh(new FileInputStream(refreshs.get(i)), refreshs.get(i).getAbsolutePath(),
                    configuration);
        }
     ...
     * @return 刷新文件列表
     */
    public List<File> getRefreshFile(final File dir, final Long beforeTime) {
        final List<File> refreshs = new ArrayList<File>();
        System.out.println("this is mapper XML Dir = " + dir);
     ...
     */
    public boolean check(final File file, final Long beforeTime) {
