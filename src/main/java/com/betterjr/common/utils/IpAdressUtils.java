...
public class IpAdressUtils {
	...
	public static String qqIpLookup(String ip) {
		...
		try {
			Document doc = Jsoup.connect(qqURL).cookie("ipqqcom_user_id", "955125")
					.userAgent("Mozilla/5.0 (compatible; MSIE 10.0; Windows NT 6.1; Trident/6.0)").data("searchip1", ip).post();
			tmpStr = doc.data();
	...
	public static IP2LocationInfo taobaoIpLookup(String ip) {
		String tmpStr = null;
		try {
			tmpStr = Jsoup.connect(taobaoURL + ip).userAgent("Mozilla/5.0 (compatible; MSIE 10.0; Windows NT 6.1; Trident/6.0)").execute().body();
	...
	public static String sinaIpLookup(String ip) {
		String tmpStr = null;
		try {
			tmpStr = Jsoup.connect(sinaURL + ip).userAgent("Mozilla/5.0 (compatible; MSIE 10.0; Windows NT 6.1; Trident/6.0)").execute().body();
