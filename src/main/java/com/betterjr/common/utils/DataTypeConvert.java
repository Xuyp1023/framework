...
public class DataTypeConvert implements Serializable {
	...
	protected static final Logger log = LoggerFactory
			.getLogger(DataTypeConvert.class);
	...
	private static Map registerTypeClass() {
		...
		Class[] arrClass = new Class[] { Boolean.TYPE, Boolean.class,
				Byte.TYPE, Byte.class, Short.TYPE, Short.class, Integer.TYPE,
				Integer.class, Long.TYPE, Long.class, Float.TYPE, Float.class,
				BigDecimal.class, BigInteger.class, Double.TYPE, Double.class,
				Character.TYPE, Character.class, String.class,
	...
	public static SimpleDateFormat[] createDateFormater() {
		String[] arrStr = new String[] { "yyyy-MM-dd HH:mm:ss",
		...
		for (int i = 0; i < arrSplit.length; i++) {
			for (int j = 0; j < arrStr.length; j++) {
				list.add(new SimpleDateFormat(arrStr[j].replaceAll("-",
						arrSplit[i])));
			}
		}
	...
	public Class findDefineClassT(String anStr) {
		...
		if (cc == null) {
			throw new DataTypeConvertException(
					"指定的数据类型不存在；类型字符是：".concat(classN));
		} else {
			return cc;
		}
	}
	...
	public Object convertT(Object value, Class target, String anPatten) {
		...
		if (Number.class.isAssignableFrom(target)) {
			...
			if (value instanceof String) {
				...
				if ((anPatten == null) || StringUtils.isBlank(anPatten)) {
					if (tmpStr.indexOf(".") > -1) {
						...
						return toNumberValue(target, new Long(
								parseLongValue(tmpStr)));
					}
				} else {
					...
					try {
						...
						throw new DataTypeConvertException(
								"错误的数字格式，和指定的模式不匹配；字符是：".concat(tmpStr)
										.concat(", 模式是：").concat(anPatten));
					}
				}
			...
			if (Number.class.isInstance(value)) {
				return ((Number) value).intValue() == 1 ? Boolean.TRUE
						: Boolean.FALSE;
			} else if (value instanceof String) {
				String tmpStr = ((String) value).toLowerCase();
				if (tmpStr.equals("true") || tmpStr.equals("1")
						|| tmpStr.equals("t")) {
					tmpStr = "true";
				} else {
					tmpStr = "false";
			...
			if ((anPatten == null) || StringUtils.isBlank(anPatten)) {
				...
				return DateFormatUtils.format(((Calendar) value).getTime(),
						anPatten);
			} else {
				return value.toString();
			}
	...
	public long convertStrToDate(final String anStr, final String anPatten) {
		if (StringUtils.isNotBlank(anPatten)) {
			try {
				...
				throw new DataTypeConvertException("错误的日期格式，和指定的模式不匹配；字符是："
						.concat(anStr).concat(", 模式是：").concat(anPatten));
			}
		}
		...
		if (ll == Long.MIN_VALUE) { // 是字符串的东西
			throw new DataTypeConvertException(
					"错误的日期格式，请与开发人员联系；字符是：".concat(anStr));
		}
