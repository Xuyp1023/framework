...
public class JedisSessionDAO extends AbstractSessionDAO implements SessionDAO {
   ...
   public void update(Session session) throws UnknownSessionException {
      ...
      try {
         ...
         String principalId = pc != null ? pc.getPrimaryPrincipal().toString() : BetterStringUtils.EMPTY;
   ...
   public Collection<Session> getActiveSessions(boolean includeLeave, Object principal, Session filterSession) {
      ...
      try {
         ...
         for (Map.Entry<String, String> e : map.entrySet()) {
            if (BetterStringUtils.isNotBlank(e.getKey()) && BetterStringUtils.isNotBlank(e.getValue())) {

               String[] ss = BetterStringUtils.split(e.getValue(), "|");
               if (ss != null && ss.length == 3) {// jedis.exists(sessionKeyPrefix
                  ...
                  try {
                     ...
                     if (includeLeave || BetterDateUtils.pastMinutes(session.getLastAccessTime()) <= 3) {
                     ...
                     if (principal != null) {
                        ...
                        if (principal.toString()
                              .equals(pc != null ? pc.getPrimaryPrincipal().toString() : BetterStringUtils.EMPTY)) {
                           isActiveSession = true;
                        }
                     }
                     // 过滤掉的SESSION
            ...
            // 存储的SESSION无Value
            else if (BetterStringUtils.isNotBlank(e.getKey())) {
               jedis.hdel(sessionKeyPrefix, e.getKey());
            }
         }
