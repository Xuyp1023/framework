...
/**
 * <p>
 * A {@link Filter} that enable client-side cross-origin requests by implementing W3C's CORS (<b>C</b>ross-<b>O</b>rigin <b>R</b>esource <b>S</b>
 * haring) specification for resources. Each {@link HttpServletRequest} request is inspected as per specification, and appropriate response headers
 ...
 * <ul>
 * <li><b>cors.isCorsRequest:</b> Flag to determine if request is a CORS request. Set to <code>true</code> if CORS request; <code>false</code>
 * otherwise.</li>
 * <li><b>cors.request.origin:</b> The Origin URL.</li>
 * <li><b>cors.request.type:</b> Type of request. Values: <code>simple</code> or <code>preflight</code> or <code>not_cors</code> or
...
public final class CORSFilter implements Filter {
    ...
    /**
     * A {@link Collection} of exposed headers consisting of zero or more header field names of headers other than the simple response headers that
     * the resource might use and can be exposed.
     */
    private final Collection<String> exposedHeaders;

    /**
     * A supports credentials flag that indicates whether the resource supports user credentials in the request. It is true when the resource does and
     * false otherwise.
     */
    ...
    public void doFilter(final ServletRequest servletRequest, final ServletResponse servletResponse,
        ...
        if (!(servletRequest instanceof HttpServletRequest) || !(servletResponse instanceof HttpServletResponse)) {
            final String message = "CORS doesn't support non-HTTP request or response.";
        ...
        final CORSFilter.CORSRequestType requestType = checkRequestType(request);
    ...
    public void init(final FilterConfig filterConfig) throws ServletException {
        ...
        if (filterConfig != null) {
            final String configAllowedOrigins = filterConfig.getInitParameter(PARAM_CORS_ALLOWED_ORIGINS);
            final String configAllowedHttpMethods = filterConfig.getInitParameter(PARAM_CORS_ALLOWED_METHODS);
            final String configAllowedHttpHeaders = filterConfig.getInitParameter(PARAM_CORS_ALLOWED_HEADERS);
            final String configExposedHeaders = filterConfig.getInitParameter(PARAM_CORS_EXPOSED_HEADERS);
            final String configSupportsCredentials = filterConfig.getInitParameter(PARAM_CORS_SUPPORT_CREDENTIALS);
            final String configPreflightMaxAge = filterConfig.getInitParameter(PARAM_CORS_PREFLIGHT_MAXAGE);
            final String configLoggingEnabled = filterConfig.getInitParameter(PARAM_CORS_LOGGING_ENABLED);
            final String configDecorateRequest = filterConfig.getInitParameter(PARAM_CORS_REQUEST_DECORATE);
     ...
     * @see <a href="http://www.w3.org/TR/cors/#resource-requests">Simple Cross-Origin Request, Actual Request, and Redirects</a>
     */
    public void handleSimpleCORS(final HttpServletRequest request, final HttpServletResponse response,
        ...
        if (!(requestType == CORSFilter.CORSRequestType.SIMPLE || requestType == CORSFilter.CORSRequestType.ACTUAL)) {
            final String message = "Expects a HttpServletRequest object of type " + CORSFilter.CORSRequestType.SIMPLE
    ...
    public void handlePreflightCORS(final HttpServletRequest request, final HttpServletResponse response,
            final FilterChain filterChain) throws IOException, ServletException {
        final CORSRequestType requestType = checkRequestType(request);
        if (requestType != CORSRequestType.PRE_FLIGHT) {
            throw new IllegalArgumentException(
                    "Expects a HttpServletRequest object of type " + CORSRequestType.PRE_FLIGHT.name().toLowerCase());
        }
        ...
        // Section 6.2.4
        final String accessControlRequestHeadersHeader = request
        ...
        if (accessControlRequestHeadersHeader != null && !accessControlRequestHeadersHeader.trim().isEmpty()) {
            final String[] headers = accessControlRequestHeadersHeader.trim().split(",");
        ...
        if (!accessControlRequestHeaders.isEmpty()) {
            for (final String header : accessControlRequestHeaders) {
    ...
    /**
     * Handles a request, that's not a CORS request, but is a valid request i.e. it is not a cross-origin request. This implementation, just forwards
     * the request down the filter chain.
     * 
     * @param request
     *            The {@link HttpServletRequest} object.
     * @param response
    ...
    public void handleInvalidCORS(final HttpServletRequest request, final HttpServletResponse response,
        ...
        final String origin = request.getHeader(CORSFilter.REQUEST_HEADER_ORIGIN);
        final String method = request.getMethod();
        final String accessControlRequestHeaders = request.getHeader(REQUEST_HEADER_ACCESS_CONTROL_REQUEST_HEADERS);
    ...
    public CORSRequestType checkRequestType(final HttpServletRequest request) {
        ...
        final String originHeader = request.getHeader(REQUEST_HEADER_ORIGIN);
            ...
            if (originHeader.isEmpty()) {
                ...
                if (method != null && HTTP_METHODS.contains(method)) {
                    if ("OPTIONS".equals(method)) {
                        final String accessControlRequestMethodHeader = request
     ...
     *            The amount of seconds the user agent is allowed to cache the result of the pre-flight request.
    ...
    private void parseAndStore(final String allowedOrigins, final String allowedHttpMethods,
        ...
        if (allowedOrigins != null) {
            if (allowedOrigins.trim().equals("*")) {
                ...
                final Set<String> setAllowedOrigins = parseStringToSet(allowedOrigins);
        ...
        if (allowedHttpMethods != null) {
            final Set<String> setAllowedHttpMethods = parseStringToSet(allowedHttpMethods);
            this.allowedHttpMethods.clear();
            this.allowedHttpMethods.addAll(setAllowedHttpMethods);
        }

        if (allowedHttpHeaders != null) {
            final Set<String> setAllowedHttpHeaders = parseStringToSet(allowedHttpHeaders);
        ...
        if (exposedHeaders != null) {
            final Set<String> setExposedHeaders = parseStringToSet(exposedHeaders);
    ...
    /**
     * The Access-Control-Allow-Origin header indicates whether a resource can be shared based by returning the value of the Origin request header in
     * the response.
     */
     ...
     * When part of the response to a preflight request it indicates that the actual request can include user credentials.
     */
    ...
    /**
     * The Access-Control-Allow-Methods header indicates, as part of the response to a preflight request, which methods can be used during the actual
     * request.
     */
    public static final String RESPONSE_HEADER_ACCESS_CONTROL_ALLOW_METHODS = "Access-Control-Allow-Methods";

    /**
     * The Access-Control-Allow-Headers header indicates, as part of the response to a preflight request, which header field names can be used during
     * the actual request.
     */
    ...
    /**
     * By default, following headers are supported: Origin,Accept,X-Requested-With, Content-Type, Access-Control-Request-Method, and
