...
class RuleDefinitionValidator {
    ...
    private void checkRuleClass(final Object rule) {
        if (!isRuleClassWellDefined(rule)) {
            throw new IllegalArgumentException(
                    format("规则服务类 '%s' 没有注解为 '%s'", rule.getClass().getName(), AnnotRuleService.class.getName()));
    ...
    private void checkConditionMethod(final Object rule) {
        ...
        if (conditionMethods.isEmpty()) {
            throw new IllegalArgumentException(
                    format("规则 '%s' 必须注解条件方法 '%s'", rule.getClass().getName(), RuleCondition.class.getName()));
        }

        for (Method conditionMethod : conditionMethods) {

            if (!isConditionMethodWellDefined(conditionMethod)) {
                throw new IllegalArgumentException(format("条件执行方法 '%s' 在规则执行类 '%s' 中，必须是公开的返回结果是Boolean,而且没有入参.",
    ...
    private void checkActionMethods(final Object rule) {
        ...
        if (actionMethods.isEmpty()) {
            throw new IllegalArgumentException(
                    format("规则 '%s' 必须有规则执行方法注解 '%s'", rule.getClass().getName(), RuleAction.class.getName()));
        }

        for (Method actionMethod : actionMethods) {
            if (!isActionMethodWellDefined(actionMethod)) {
                throw new IllegalArgumentException(
                        format("规则执行方法 '%s' 在规则执行类 '%s' 中，必须是公开的无参数和返回结果.", actionMethod, rule.getClass().getName()));
            }
        }
    }
