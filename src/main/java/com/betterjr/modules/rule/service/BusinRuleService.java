...
public class BusinRuleService extends BaseService<RuleBusinessMapper, RuleBusiness> {
     ...
     * @throws 异常情况
     */
    public List<RuleFunction> annotBuild(Map<String, Object> ruleClasses, Map<String, RuleBusiness> tmpBusinMap, Map<String, RuleFace> anRuleMap) {
...
//            final Class<? extends Object> ruleClass = rule.getClass();
            ...
            if (annot.type() == RuleServiceType.RULE) {
                if (rule instanceof BasicRule) {
                    ...
                    throw new BettjerRuleException(40100, String.format("Annotation Rule Class '%s', Must implements Interface RuleObjectFace",
                            ruleClass.getName()));
                }
...
//            final Class<? extends Object> ruleClass = rule.getClass();
                        ...
                        if (subAnnot instanceof AnnotRuleProcess) {
                            ...
                            for (String tmpStr : arp.rules()) {
                                ...
                                if (ruleObj != null) {
                                    ...
                                    throw new BettjerRuleException(40100, String.format(
    ...
    public RuleCheckResult execute(RuleBusiness ruleBusin, ExpressRunner runner, QLExpressContext context) throws Exception {
        ...
        if (ruleBusin != null) {
            ...
            if (result.workContinue(ruleBusin.getExecContent())) {
                Object resultObj = runner.execute(ruleBusin.getExecContent(), context, result.getErrorList(), true, false);
     ...
     * @throws 异常情况
     */
    private RuleCheckResult executeRules(RuleCheckResult result, ExpressRunner runner, List<RuleFace> rules, QLExpressContext ruleContext) {
        try {
            boolean hasOk = false;
    ...
    private boolean workSingleRule(RuleCheckResult anResult, Object subObj, ExpressRunner runner, QLExpressContext ruleContext) throws Exception {
        WorkRuleInfo rule;
        if (subObj instanceof WorkRuleInfo) {
            rule = (WorkRuleInfo) subObj;
            // 检查规则的有效性
            if (rule.checkValid()) {
                Boolean result = (Boolean) runner.execute(rule.getMatchCond(), ruleContext, anResult.getErrorList(), true, false);
                if (result != null && result.booleanValue()) {
                    Object resultObj = runner.execute(rule.getExecContent(), ruleContext, anResult.getErrorList(), true, false);
