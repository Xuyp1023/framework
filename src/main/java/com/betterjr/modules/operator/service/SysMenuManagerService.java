...
public class SysMenuManagerService {
    ...
    public List findRoleByOperator(String anRoleId) {
        ...
        if (StringUtils.isNotBlank(anRoleId)) {
            tmpList.add(anRoleId);
        } else {
            CustOperatorInfo operator = UserUtils.getOperatorInfo();
            if (operator.getDefOper() && StringUtils.isNotBlank(operator.getRuleList())) {
                tmpList = UserUtils.findRuleList();
            } else {
                String[] arrRule = StringUtils
                        .split(operatorRoleRelationService.findSysRoleByOperatorId(operator.getId()), ";|,");
    ...
    public void addMenuRole(String anRoleId, String anRoleName, String anMenuIdArr) {
        if (StringUtils.isNotBlank(anMenuIdArr)) {
            // 先删除所有角色菜单
            sysMenuRuleService.delMenuRole(Long.parseLong(anRoleId));
            String[] menuArr = StringUtils.split(anMenuIdArr, ";|,");
