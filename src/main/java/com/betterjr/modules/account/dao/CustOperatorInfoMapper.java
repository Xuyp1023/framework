...
public interface CustOperatorInfoMapper extends Mapper<CustOperatorInfo> {
    ...
    public CustOperatorInfo findCustOperatorByIndentInfo(@Param("identType") String anIndentType,
            @Param("identNo") String anIndetNo);

    @Select("select a.* from T_CUST_OPERATOR a where  a.C_STATUS ='1' and a.C_OPERORG = #{operOrg} and a.C_CODE = #{operCode}")
    @ResultType(CustOperatorInfo.class)
    public CustOperatorInfo findCustOperatorByOperCode(@Param("operOrg") String anOperOrg,
            @Param("operCode") String anOperCode);
}