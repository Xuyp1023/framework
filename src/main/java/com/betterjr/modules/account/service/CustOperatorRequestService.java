...
public class CustOperatorRequestService extends BaseService<CustOperatorInfoMapper, CustOperatorInfo> {
    ...
    public CustOptData saveCustOperator(Map anMap) {
        ...
        boolean optExists = this.custOptService.checkOperatorExists(request.getContIdentType(),
                request.getContIdentNo());
        if (optExists) {
            throw new BytterTradeException(40001, "抱歉，该证件号码已存在");
        }
        // 判断该操作员是否存在
        boolean operCodeExists = this.custOptService.checkExistsByOperCodeAndOperOrg(request.getOperCode(),
                request.getOperOrg());
        if (operCodeExists) {
    ...
    public CustOptData updateCustOperator(Map anMap) {
        ...
        if (StringUtils.isBlank(anRecord.getRuleList())) {
            logger.error("角色不能为空");
            throw new BytterTradeException(40001, "抱歉，角色不能为空");
        }
    ...
    public Page<CustOptData> queryCustOperatorByPage(Map<String, String> anMap) {
        int pageNum = anMap.get("pageNum") == null || "".equals(anMap.get("pageNum")) ? 1
                : Integer.valueOf(anMap.get("pageNum"));
        int pageSize = anMap.get("pageSize") == null || "".equals(anMap.get("pageSize")) ? 20
