...
public class ExpressParse {
    ...
    public ExpressNode parse(ExpressPackage rootExpressPackage, String express, boolean isTrace,
        ...
        QLMatchResult result = QLPattern.findMatchStatement(this.nodeTypeManager,
                this.nodeTypeManager.findNodeType("PROGRAM").getPatternNode(), tempList, 0);
        if (result == null) {
        ...
        if (result.getMatchLastIndex() < tempList.size()) {
            ...
            throw new Exception("还有单词没有完成语法匹配：" + result.getMatchLastIndex() + "[" + tempNode.getValue() + ":line="
                    + tempNode.getLine() + ",col=" + tempNode.getCol() + "] 之后的单词 \n" + express);
        }
