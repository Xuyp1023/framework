...
public class QLPattern {
    ...
    private static QLMatchResult findMatchStatementWithAddRoot(INodeTypeManager aManager, QLPatternNode pattern,
            List<? extends IDataNode> nodes, int point, boolean isRoot, AtomicLong maxMatchPoint) throws Exception {
    ...
    private static QLMatchResult matchDetailOneTime(INodeTypeManager aManager, QLPatternNode pattern,
            List<? extends IDataNode> nodes, int point, AtomicLong maxMatchPoint) throws Exception {
        QLMatchResult result = null;
        ...
        } else if (pattern.nodeType == aManager.findNodeType("EOF") && point < nodes.size()
                && nodes.get(point).getValue().equals("}")) {
    ...
    private static QLMatchResult matchOrOneTime(INodeTypeManager aManager, QLPatternNode pattern,
            List<? extends IDataNode> nodes, int point, AtomicLong maxMatchPoint) throws Exception {
        QLMatchResult result = null;
    ...
    private static QLMatchResult matchAndOneTime(INodeTypeManager aManager, QLPatternNode pattern,
            List<? extends IDataNode> nodes, int point, AtomicLong maxMatchPoint) throws Exception {
        int orgiPoint = point;
        ...
        for (QLPatternNode item : pattern.children) {
            ...
            if (tempResult != null) {
                ...
                if (item.isTreeRoot == true && tempResult.matchs.size() > 0) {
                    if (tempResult.matchs.size() > 1) throw new Exception("根节点的数量必须是1");
                    if (root == null) {
    ...
    public static void traceLog(QLPatternNode pattern, QLMatchResult result, List<? extends IDataNode> nodes, int point,
            int matchCount) {
        if (log.isTraceEnabled() && (pattern.matchMode == MatchMode.DETAIL || pattern.matchMode == MatchMode.AND
                && matchCount > 1 && pattern.name.equals("ANONY_PATTERN") == false)) {
