...
public class QLPatternNode {
    ...
    public void splitChild() throws Exception {
        ...
        if (orgStr.equals("(") || orgStr.equals(")") || orgStr.equals("|") || orgStr.equals("||")
        ...
        for (int i = 0; i < orgStr.length(); i++) {
            if (orgStr.charAt(i) == '(') {
                ...
                if (this.matchMode != MatchMode.NULL && this.matchMode != MatchMode.AND) {
                    throw new Exception("不正确的模式串,在一个匹配模式中不能|,$并存,请使用字串模式:" + orgStr);
                }
        ...
        if (tempStr.length() > 2 && tempStr.charAt(0) == '(' && tempStr.charAt(tempStr.length() - 1) == ')') {
            ...
            this.children.add(new QLPatternNode(this.nodeTypeManager, "ANONY_PATTERN",
                    tempStr.substring(1, tempStr.length() - 1), false, this.level + 1));
            this.matchMode = MatchMode.AND;
            tempStr = "";
