...
public class IfInstructionFactory extends InstructionFactory {
    ...
    public boolean createInstruction(ExpressRunner aCompile, InstructionSet result, Stack<ForRelBreakContinue> forStack,
        ...
        for (int i = 0; i < oldChildren.length; i++) {
            if (oldChildren[i].isTypeEqualsOrChild("then") || oldChildren[i].isTypeEqualsOrChild("else")
                    || oldChildren[i].isTypeEqualsOrChild("?") || oldChildren[i].isTypeEqualsOrChild(":")) {
                continue;
            }
        ...
        result.insertInstruction(finishPoint[0] + 1,
                new InstructionGoToWithCondition(false, result.getCurrentPoint() - finishPoint[0] + 2, true));
        ...
        result.insertInstruction(finishPoint[1] + 1,
                new InstructionGoTo(result.getCurrentPoint() - finishPoint[1] + 1));
        return r1 || r2 || r3;
    }
}
