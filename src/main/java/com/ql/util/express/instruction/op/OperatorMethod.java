    ...
    public OperateData executeInner(InstructionSetContext parent, OperateData[] list) throws Exception {
        ...
        if (obj == null) {
            ...
            if (list[0] instanceof OperateClass) {// 调用静态方法
                m = ExpressUtil.findMethodWithCache((Class<?>) obj, methodName, types, true, true);
            } else {
                m = ExpressUtil.findMethodWithCache(obj.getClass(), methodName, types, true, false);
            }
            if (m == null) {
            ...
            if (m == null) {
                ...
                s.append("没有找到" + obj.getClass().getName() + "的方法：" + methodName + "(");
